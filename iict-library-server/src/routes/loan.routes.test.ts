import express from 'express';
import request from 'supertest';
import { beforeEach, describe, expect, it, vi } from 'vitest';
import { Role } from '@prisma/client';

const mocks = vi.hoisted(() => ({
  prisma: {
    user: { findUnique: vi.fn(), create: vi.fn(), update: vi.fn() },
    studentProfile: { findUnique: vi.fn(), create: vi.fn() },
    adminProfile: { findUnique: vi.fn(), create: vi.fn() },
    teacherProfile: { findUnique: vi.fn(), create: vi.fn() },
  },
}));

vi.mock('../config/database', () => ({ default: mocks.prisma }));

const { default: loanRouter } = await import('./loan.routes');

const app = express();
app.use(express.json());
app.use('/loans', loanRouter);

beforeEach(() => {
  vi.clearAllMocks();
  process.env.ENABLE_DEV_AUTH = 'true';
  process.env.NODE_ENV = 'test';
});

describe('loan routes RBAC', () => {
  it('rejects issue attempts by non-admin users', async () => {
    mocks.prisma.user.findUnique.mockResolvedValue({
      id: 'student-user',
      email: 'student@example.com',
      name: 'Student One',
      role: Role.STUDENT,
    });
    mocks.prisma.studentProfile.findUnique.mockResolvedValue({
      id: 'student-profile',
      userId: 'student-user',
    });

    const response = await request(app)
      .post('/loans/issue')
      .set('x-user-id', 'student@example.com')
      .set('x-user-role', 'STUDENT')
      .send({ accessionNumber: 'ACC-1', userId: 'student-user' });

    expect(response.status).toBe(403);
    expect(response.body.message).toBe('Forbidden');
  });

  it('allows my history when an existing dev user switches to Student role', async () => {
    mocks.prisma.user.findUnique.mockResolvedValue({
      id: 'dev-user',
      email: 'dev@example.com',
      name: 'Dev User',
      role: Role.ADMIN,
    });
    mocks.prisma.user.update.mockResolvedValue({
      id: 'dev-user',
      email: 'dev@example.com',
      name: 'Dev User',
      role: Role.STUDENT,
    });
    mocks.prisma.studentProfile.findUnique.mockResolvedValue({
      id: 'student-profile',
      userId: 'dev-user',
    });

    const response = await request(app)
      .get('/loans/history/me')
      .set('x-user-id', 'dev@example.com')
      .set('x-user-role', 'STUDENT');

    expect(response.status).not.toBe(403);
    expect(mocks.prisma.user.update).toHaveBeenCalledWith({
      where: { id: 'dev-user' },
      data: { role: Role.STUDENT },
    });
  });
});
